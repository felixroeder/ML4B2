...
news_data = pd.read_csv('/content/Datensatz.csv')  # Replace with your dataset path
...
# Combine features into a single array for each company
def combine_features(features):
    # Adjust the reshape dimensions as needed to match your feature structure
    combined = np.concatenate([f.reshape(f.shape[0], -1) for f in features], axis=1)
    return combined

# Ensure sequences and targets are of consistent length before concatenating
consistent_length = min(min(len(sequences) for sequences, _ in company_sequences.values()),
                        min(len(targets) for _, targets in company_sequences.values()))

company_sequences = {ticker: (sequences[:consistent_length], targets[:consistent_length]) for ticker, (sequences, targets) in company_sequences.items()}

# Convert sequences to arrays for model input
company_features = {ticker: (convert_sequences(sequences), targets) for ticker, (sequences, targets) in company_sequences.items()}

# Validate lengths of the features and ensure consistency
for key, (value, targets) in company_features.items():
    print(f"{key} lengths: {[len(x) for x in value]}, targets length: {len(targets)}")
...
targets_array = np.concatenate([targets for _, targets in company_features.values()], axis=0)
...
targets_df = pd.DataFrame(targets_array)

# Separate scalers for different feature types
price_scaler = StandardScaler()
other_features_scaler = StandardScaler()

# Fit and transform the stock prices
price_data = combined_features_array[:, :look_back]  # Adjust indexing if needed
price_data_scaled = price_scaler.fit_transform(price_data.reshape(-1, 1)).reshape(price_data.shape)

# Fit and transform other features
other_features_data = combined_features_array[:, look_back:]  # Adjust indexing if needed
other_features_data_scaled = other_features_scaler.fit_transform(other_features_data.reshape(-1, other_features_data.shape[-1])).reshape(other_features_data.shape)

# Combine scaled features
combined_features_array_scaled = np.concatenate([price_data_scaled, other_features_data_scaled], axis=-1)

# Split the data
X_train, X_val, y_train, y_val = train_test_split(combined_features_array_scaled, targets_df.values, test_size=0.2, random_state=42)
...
# Verify the target values in X_train
#for i in range(5):
#    print(f"Target {i}: {y_train[i]}")
#    print(f"Corresponding prices in X_train {i}: {X_train[i, :, 0]}")  # Assuming the first feature is the price

# Fit and transform the stock prices
price_data = combined_features_array[:, :1]  # Assuming the first feature is the price
price_data_scaled = price_scaler.fit_transform(price_data.reshape(-1, 1)).reshape(price_data.shape)

# Fit and transform other features
other_features_data = combined_features_array[:, 1:]
other_features_data_scaled = other_features_scaler.fit_transform(other_features_data.reshape(-1, other_features_data.shape[-1])).reshape(other_features_data.shape)
...
combined_dim = combined_features_array_scaled.shape[-1]  # Combined dimension
model = create_keras_model(look_back, combined_dim, num_companies, 12, 128, 0.5)

# Ensure the number of samples is the same
if combined_features_array_scaled.shape[0] != targets_df.shape[0]:
    min_samples = min(combined_features_array_scaled.shape[0], targets_df.shape[0])
    combined_features_array_scaled = combined_features_array_scaled[:min_samples]
...
# Inverse transform the predictions for each company
predicted_prices = {}
for i, ticker in enumerate(companies_to_focus.keys()):
    predicted_prices[ticker] = price_scaler.inverse_transform(predicted_prices_scaled[i])

# Display the predicted prices
for ticker, prices in predicted_prices.items():
    print(f"{ticker} predicted prices:\n", prices)
